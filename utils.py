...
def myload(dataset_name='douban', edge_noise=0.):
    ...
    if dataset_name in ['douban']:
        ...
        Afeat = np.load('dataset/movie/node_type0.npy', allow_pickle=True)
        Bfeat = np.load('dataset/movie/node_type1.npy', allow_pickle=True)
        Aadj = np.load('dataset/movie/adj0.npy', allow_pickle=True)
        Badj = np.load('dataset/movie/adj1.npy', allow_pickle=True)
        ground_truth = np.loadtxt('dataset/movie/true_matching').astype('int32')
...
def load_final(dataset_name):
	x = loadmat(os.path.join('dataset', 'douban', '{name}.mat'.format(name=dataset_name)))
